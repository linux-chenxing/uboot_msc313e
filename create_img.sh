...
out_file=u-boot.mz.img.bin
out_file_xz=u-boot.xz.img.bin
if [ `echo $ms_ver | grep -c "MVX1S" ` -gt 0 ];then
  out_file=u-boot_S.mz.img.bin
  out_file_xz=u-boot_S.xz.img.bin
fi

echo ""
echo $out_file
echo ./mkimage -A arm -O u-boot -C mz -a 0 -e 0 -n "$(echo $ms_ver)" -d u-boot.bin.mz "$out_file"
./mkimage -A arm -O u-boot -C mz -a 0 -e 0 -n "$ms_ver" -d u-boot.bin.mz "$out_file"
rm -Rf u-boot.bin.mz
echo ""
