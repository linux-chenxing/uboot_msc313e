...
void _PnlClkStore(PnlSysFsStrConfig_t *pstStringCfg)
{
    ...
    else
    {
        ...
        PNLSYSFS_SPRINTF_STRCAT(SrcBuf, "clktree [En] [%s 0 ~ %d]\n", au8ClkMuxAttr[i] ? "CLKIdx": "ClkRate", HAL_PNL_CLK_NUM-1);
        PNLSYSFS_SPRINTF_STRCAT(SrcBuf, "----------------- DRV Update -----------------\n");
        PNLSYSFS_SPRINTF_STRCAT(SrcBuf, "[CLK Type] [En] [%s]\n",  au8ClkMuxAttr[i] ? "CLKIdx": "ClkRate");
        PNLSYSFS_SPRINTF_STRCAT(SrcBuf, "----------------- CLK Type ----------------- \n");
        for(i=0; i< HAL_PNL_CLK_NUM; i++)
        {
            PNLSYSFS_SPRINTF_STRCAT(SrcBuf, "%s, \n", au8ClkName[i]);
        }
        PNLSYSFS_DBG("%s\n", SrcBuf);
        DrvPnlOsMemRelease(SrcBuf);
        return;
