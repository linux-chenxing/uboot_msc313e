...
{
    u32 u32Mode = PINMUX_FOR_UNKNOWN_MODE;

if ((eLink == E_HAL_PNL_LINK_TTL) || (eLink == E_HAL_PNL_LINK_TTL_SPI_IF))
    {
...
void __HalPnlIfSetPolarityConfig(DrvPnlCtxConfig_t *pPnlCtx, HalPnlParamConfig_t *pParamCfg)
{
    ...
    if (pParamCfg->eLinkType == E_HAL_PNL_LINK_TTL ||  pParamCfg->eLinkType == E_HAL_PNL_LINK_TTL_SPI_IF) {
        ...
        HalPnlSetBt656ClkInv(pstPolarityCfg->bClkInv);
        HalPnlSetVsynRefMd(0);
    }
}
...
void __HalPnlIfSetLpllConfig(DrvPnlCtxConfig_t *pPnlCtx,  HalPnlParamConfig_t *pParamCfg)
{
    ...
    if(u16LpllIdx == 0xFF) {
        PNL_ERR( "[HALPNL]%s %d:: wrong lpll table index %d caused by unsupported clock range, set clock failed \n",__FUNCTION__, __LINE__, u16LpllIdx);
        return;
    }
    ...
    if( (u16LpllIdx == 6) ||
        (u16LpllIdx == 5))
    {
        if(u32LplLSet < 0x228f5c || u32LplLSet > 0x451eb8)
        {
            PNL_ERR( "[HALPNL]%s %d:: wrong lpll range %x, idx %d\n",
                    __FUNCTION__, __LINE__, (int)u32LplLSet,u16LpllIdx);
            return;
        }
    }
    if(u16LpllIdx == 4)
    {
        if(u32LplLSet < 0x3c2fa6 || u32LplLSet > 0x451eb8)
        {
            PNL_ERR( "[HALPNL]%s %d:: wrong lpll range %x, idx %d\n",
                    __FUNCTION__, __LINE__, (int)u32LplLSet,u16LpllIdx);
            return;
    ...
    PNL_DBG(PNL_DBG_LEVEL_HAL, "[HALPNL]%s %d:: Idx:%d, LoopGain:%d, LoopDiv:%d, fps=%ld, Divden:0x%lx, Divisor:0x%lx, LpllSe:0x%x\n",
                            __FUNCTION__, __LINE__, u16LpllIdx,
        pstLpllCfg->u16LoopGain, pstLpllCfg->u16LoopDiv, pstLpllCfg->u32Fps,
        u32Dividen, u32Divisor, (int)u32LplLSet);
    HalPnlDumpLpllSetting(u16LpllIdx);
    HalPnlSetLpllSet((u32)u32LplLSet);
}
...
void __HalPnlIfUnifiedSetLpllConfig(DrvPnlCtxConfig_t *pPnlCtx, u32 u32OutClkRate)
{
    ...
    u32Dividen = ((u32)432 * (u32)524288 * (u32)pstLpllCfg->u16LoopGain) / (u32)pstLpllCfg->u16LoopDiv;
    u32Divisor = pstLpllCfg->u32Dclk / 1000;
    u32LplLSet= u32Dividen / u32Divisor;
    u32LplLSet = u32LplLSet * 1000;
    if( (u16LpllIdx == 6) ||
        (u16LpllIdx == 5))
    {
        if(u32LplLSet < 0x228f5c || u32LplLSet > 0x451eb8)
...
void __HalPnlIfSetTimeGenConfig(DrvPnlCtxConfig_t *pPnlCtx,  HalPnlParamConfig_t *pParamCfg)
{
    ...
    if(pParamCfg->eLinkType == E_HAL_PNL_LINK_TTL ||  pParamCfg->eLinkType == E_HAL_PNL_LINK_TTL_SPI_IF ) {
        ...
        HalPnlEnableBT656(0);
        HalPnlSetY2RMatrix(1);
    }
    else if (pParamCfg->eLinkType == E_HAL_PNL_LINK_BT656) {
        u16VsyncSt =0;
        HalPnlSetVSyncSt(u16VsyncSt);
        HalPnlSetVSyncEnd(u16VsyncSt + pstTimeGenCfg->u16VsyncWidth - 1);
        u16VdeSt = u16VsyncSt + pstTimeGenCfg->u16VsyncWidth + pstTimeGenCfg->u16VsyncBackPorch;
        HalPnlSetVfdeSt(u16VdeSt);
        HalPnlSetVfdeEnd(u16VdeSt + pstTimeGenCfg->u16Vactive - 1);
        HalPnlSetVdeSt(u16VdeSt);
        HalPnlSetVdeEnd(u16VdeSt + pstTimeGenCfg->u16Vactive - 1);
        ...
        HalPnlEnableBT656(1);
        HalPnlSetY2RMatrix(0);
        PNL_DBG(PNL_DBG_LEVEL_HAL, "[HALPNL]%s %d:: BT656 Hsync(%d %d), Vsync(%d %d) SyncStart(%d %d), DeStart(%d, %d) Size(%d %d), Total(%d %d)\n",
                    __FUNCTION__, __LINE__,
                    pstTimeGenCfg->u16HsyncWidth, pstTimeGenCfg->u16HsyncBackPorch,
                    pstTimeGenCfg->u16VsyncWidth, pstTimeGenCfg->u16VsyncBackPorch,
                    u16HsyncSt, u16VsyncSt, u16HdeSt, u16VdeSt,
...
void _HalPnlIfSetUnifiedParamConfig(void *pCtx, void *pCfg)
{
    ...
    if(pstUfdParamCfg->u8TgnPadMuxFlag)
    {
        ...
        HalPnlSetUnifiedChipTop(u32Mode); // pad mux
    }
    //set DCLK
