        				...
        				case HDCP14Tx_SubFSM_ExchangeKSV:
                            {
            					...
            					{
            					    DrvHdmitxOsMsDelay(HDCP_DDC_SAMPLE_PERIOD);
...
static void _HDCPTx_Task(void) //wilson@kano
{
    ...
    while (g_bHdcpTxTask == TRUE)
    {
        ...
        DrvHdmitxOsMsDelay(HDCPTX_MONITOR_DELAY);
    } // Task loop
}
...
void MDrv_HDMITx_Set_VS_InfoFrame(void* pInstance, MsHDMITX_VIDEO_VS_FORMAT enVSFmt, MsHDMITX_VIDEO_4k2k_VIC enVid4K2KVic, MsHDMITX_VIDEO_3D_STRUCTURE enVid3DStruct)
{
    ...
    DrvHdmitxOsMsDelay(20); //delay 1 frame/field interval
...
MS_BOOL  MDrv_HDMITx_SetDbgLevel(MS_U16 u16DbgSwitch)
{
    ...
    MHal_HDMITx_DebugEnable(_u16DbgHDMITXSwitch & HDMITX_DBG_HAL ? TRUE : FALSE);
    DrvHdmitxOsSetDebugEnable(_u16DbgHDMITXSwitch & HDMITX_DBG_OS ? TRUE : FALSE);
    return TRUE;
}
            	...
            	case E_HDMITX_FSM_DONE:
                    {
                        ...
                        // Don't send unused Event to avoid cpu usage becomes too high
                        //DrvHdmitxOsSetEvent(_s32HDMITxEventId, E_HDMITX_EVENT_RUN);
                   }
                break;
...
static void _HDMITx_Task(void* pInstance) //wilson@kano
{
    ...
    while (g_bHDMITxTask == TRUE)
    {
        ...
        DrvHdmitxOsWaitEvent(_s32HDMITxEventId,
            E_HDMITX_EVENT_RUN |
            E_HDMITX_EVENT_IRQ |
            E_HDMITX_EVENT_RXTIMER |
            E_HDMITX_EVENT_CECRX |
            E_HdcpTX_EVENT_RUN |
            E_HdcpTX_EVENT_IRQ |
            E_HdcpTX_EVENT_RITIMER,
            &u32Events,
            E_DRV_HDMITX_OS_EVENT_MD_OR_CLEAR,
            HDMITX_OS_WAIT_FOREVER);
