...
#define DBG_OS_HDMITX(_f)     do{ if(_gbHdmitxOsDbgEn){ (_f);} } while(0)
//-------------------------------------------------------------------------------------------------
//  Structure
//-------------------------------------------------------------------------------------------------
typedef struct
...
MS_BOOL DrvHdmitxOsGetMmioBase(MS_VIRT *pu32BaseAddr, MS_PHY *pu32BaseSize, DrvHdmitxOsMmioType_e enType)
{
    ...
    switch(enType)
    {
        case E_HDMITX_OS_MMIO_PM:
            *pu32BaseAddr = HDMITX_OS_MMIO_PM_BASE;
            *pu32BaseSize = HDMITX_OS_MMIO_PM_SIZE;
             break;

        case E_HDMITX_OS_MMIO_NONEPM:
            *pu32BaseAddr = HDMITX_OS_MMIO_NONEPM_BASE;
...
void DrvHdmitxOsMsDelay(MS_U32 u32Msec)
{
    CamOsMsDelay(u32Msec);
}

void DrvHdmitxOsUsDelay(MS_U32 u32Usec)
{
...
u8 Hdmitx_ClkRegister(struct device_node *device_node)
{
    ...
    {
        HdmitxParentCnt /= sizeof(int);
        //printk( "[%s] Number : %d\n", __FUNCTION__, num_parents);
        if(HdmitxParentCnt < 0)
        {
            DBG_OS_HDMITX(CamOsPrintf( "[%s] Fail to get parent count! Error Number : %d\n", __FUNCTION__, HdmitxParentCnt));
            return 0;
        }
...
MS_BOOL DrvHdmitxOsSetClkOn(void *pClkRate, MS_U32 u32ClkRateSize)
{
    ...
    {
        Hdmitx_ClkRegister(gpHdmitxDeviceNode);
        for(u32clknum = 0; u32clknum < HdmitxParentCnt; u32clknum++)
        {
            if (pvHdmitxclk[u32clknum])
            {
    ...
    if(gpHdmitxDeviceNode && pClkRate && u32ClkRateSize == HAL_HDMITX_CLK_NUM)
    {
        ...
        if(num_parents ==  HAL_HDMITX_CLK_NUM)
        {
            ...
            for(idx=0; idx<num_parents; idx++)
            {
                ...
                DBG_OS_HDMITX(CamOsPrintf("%s %d, [Hdmitx] num_parents:%d-%d %15s %ld\n" ,
                              __FUNCTION__, __LINE__,
                              num_parents, idx+1,
...
MS_BOOL DrvHdmitxOsSetClkOff(void *pClkRate, MS_U32 u32ClkRateSize)
{
    MS_BOOL bRet = TRUE;
    MS_U32 num_parents, idx;
    struct clk **hdmitx_clks;
    ...
    if(gpHdmitxDeviceNode)
    {
        ...
        if(num_parents == HAL_HDMITX_CLK_NUM)
        {
            ...
            for(idx=0; idx<num_parents; idx++)
            {
                ...
                {
                    DBG_OS_HDMITX(CamOsPrintf("%s %d, [Hdmitx] %d-%d, %15s Off\n",
...
void DrvHdmitxOsSetDebugEnable(MS_BOOL bEn)
{
    _gbHdmitxOsDbgEn = bEn;
}
