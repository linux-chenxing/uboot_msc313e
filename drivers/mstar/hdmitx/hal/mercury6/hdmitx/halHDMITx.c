...
//  Debug Functions
//-------------------------------------------------------------------------------------------------
static MS_BOOL _bDbgHalHdmitxEn= 0;
#define DBG_HAL_HDMITX(_f)     do{ if(_bDbgHalHdmitxEn){ (_f);} } while(0)
...
#define PARSING_ATOP_IDX(x)  (  x ==HAL_HDMITX_ID_480_60P       ?  "480_60P" : \
                                ...
                                x ==HAL_HDMITX_ID_2560x1440_30P ?  "2560x1440_30P" : \
                                                                   "UNKWON")
    ...
    switch(packet_type)
    {
        case E_HDMITX_VS_INFOFRAME:
            for (i=0; i < length; i++)
            {
                if (i >= REG_HDMITX_PKT_VS_LEN)
                {
                    ...
                    DBG_HAL_HDMITX(printf("VS packet over size, length = %d \n", length));
                    break;
                }
        ...
        case E_HDMITX_AVI_INFOFRAME:
            for ( i = 0; i < length; i++ )
            {
                ...
                {
                    DBG_HAL_HDMITX(printf("AVI packet over size, length = %d \n", length));
                    break;
                }
        ...
        default:
            bRet = FALSE;
            DBG_HAL_HDMITX(printf("Not implemented, packet type = %u\n", packet_type));
        break;
    }
...
{
    DBG_HAL_HDMITX(printf("[%s][%d]HDMI mode = %d, Color Depth = %d \n", __FUNCTION__, __LINE__, bflag, cd_val));
...
void MHal_HDMITx_DumpAtopTable(MsHDMITX_VIDEO_TIMING video_idx, MsHDMITX_VIDEO_COLORDEPTH_VAL cd_val, MS_BOOL bIs420, MS_BOOL bCommon)
{
    ...
    switch(video_idx)
    {
        ...
        case E_HDMITX_RES_2560x1440p_30Hz:
            u16TopId = HAL_HDMITX_ID_2560x1440_30P;
            break;
...
void MHal_HDMITx_SetAnalogDrvCur(MsHDMITX_ANALOG_TUNING *pInfo)
{
    ...
    {
        DBG_HAL_HDMITX(printf("%s %d, DrvCur Tap1(%x %x %x %x) Tap2(%x %x %x %x)",
...
void MHal_HDMITx_SetVideoOutputMode(MsHDMITX_VIDEO_TIMING idx, MS_BOOL bflag, MsHDMITX_VIDEO_COLORDEPTH_VAL cd_val, MsHDMITX_ANALOG_TUNING *pInfo, MS_U8 ubSSCEn)
{
    ...
    DBG_HAL_HDMITX(printf("video idx = %d, color depth = %d \r\n", idx, cd_val));
    ...
    {
        DBG_HAL_HDMITX(printf("%s %d, Hdmi20 not support\n", __FUNCTION__, __LINE__));
    }
...
{
    DBG_HAL_HDMITX(printf("_gHPDGpioPin = 0x%x\r\n", u8pin));
...
MS_BOOL MHal_HDMITx_SetClk(MS_BOOL *pbEn, MS_U32 *pu32ClkRate, MS_U32 u32Size)
{
    ...
    if(pbEn && pu32ClkRate && u32Size == HAL_HDMITX_CLK_NUM)
    {
        ...
        MHal_HDMITx_Mask_Write(HDMITX_CLKGEN_BASE, REG_HDMITX_CLK_DISP_432, 0x0F00, u16RegVal);
    }
    else
    {
        bRet = FALSE;
...
void MHal_HDMITx_DebugEnable(MS_BOOL bEn)
{
    _bDbgHalHdmitxEn = bEn;
}
