...
void* _HalDispUtilityGetCmdqCmdBuffer(HalDispUtilityCmdqContext_t *pCtx)
{
    return pCtx->pvCmdqCmdBuffer;
}

void* _HalDispUtilityGetCmdqCmdCnt(HalDispUtilityCmdqContext_t *pCtx)
{
    return (void *)&pCtx->u32CmdqCmdCnt;
}
...
void _HalDispUtilityDirectCmdqWriteWithCmdqCmd(void *pvCtxIn)
{
    ...
    pvDirctCmdqBuffer = _HalDispUtilityGetDirectCmdqBuffer(pCtx);
    pu32DirectCmdqCnt  = _HalDispUtilityGetDirectCmdqCnt(pCtx);

    pvCmdqCmdBuffer = _HalDispUtilityGetCmdqCmdBuffer(pCtx);
    pu32CmdqCmdCnt = _HalDispUtilityGetCmdqCmdCnt(pCtx);
...
void HalDispUtilityWBYTEMSKInquireBuf(u32 u32Reg, u16 u16Val, u16 u16Mask, void *pCtx)
{
    ...
    HalDispUtilityW2BYTEMSKDirectCmdq(pCtx,
    ...
    if (pvDirctCmdqBuffer && *pu32DirectCmdqCnt > 0)
    {
        if (pstCmdqInf && pstCmdqInf->MHAL_CMDQ_WriteRegCmdqMaskMulti)
        {
            DISP_DBG(DISP_DBG_LEVEL_UTILITY_FLIP, "%s %d, Use Cmdq_%ld \n",
                __FUNCTION__, __LINE__, pCtx->s32CmdqId);
            ...
            for (i = 0, j = 0; i < *pu32DirectCmdqCnt; i++)
            {
                if(*pu32CmdqCmdCnt)
                {
                    ...
                    if(pstCmdqCmdCur->u32CmdqBufIdx == i)
                    {
                        if(pstCmdqCmdCur->u32Addr == HAL_DISP_UTILITY_CMDQ_DELAY_CMD)
                        {
                            DISP_DBG(DISP_DBG_LEVEL_UTILITY_FLIP, "%s %d, DelayCmd\n",
                                __FUNCTION__, __LINE__);
                            HalDispUtilityAddDelayCmd(pvCtxIn, pstCmdqCmdCur->u32Time);
                        }
                        else
                        {
                            ...
                            HalDispUtilityPollWait(pvCtxIn,
                                pstCmdqCmdCur->u32Addr,
                                pstCmdqCmdCur->u16Data,
                                pstCmdqCmdCur->u16Mask,
                                pstCmdqCmdCur->u32Time);
...
void HalDispUtilityWritePollCmd(u32 u32Reg, u16 u16Val, u16 u16Mask, u32 u32PollTime, void *pvCtxIn)
{
    HalDispUtilityCmdqCmd_t *pstCmdCmd = NULL;
    u32 *pu32DirectCmdqCnt = NULL;
    u32 *pu32CmdqCmdCnt = NULL;
    void *pvTblBuffer = NULL;
    HalDispUtilityCmdqContext_t *pCtx = (HalDispUtilityCmdqContext_t *)pvCtxIn;

    pvTblBuffer = _HalDispUtilityGetCmdqCmdBuffer(pCtx);
    pu32DirectCmdqCnt  = _HalDispUtilityGetDirectCmdqCnt(pCtx);
    pu32CmdqCmdCnt = _HalDispUtilityGetCmdqCmdCnt(pCtx);
    ...
    if (HalDispUtilityGetRegAccessMode() ==  E_DISP_UTILITY_REG_ACCESS_CMDQ)
    {
        ...
        pstCmdCmd = (HalDispUtilityCmdqCmd_t *)(pvTblBuffer + ((*pu32CmdqCmdCnt) * sizeof(HalDispUtilityCmdqCmd_t)));
        pstCmdCmd->u32CmdqBufIdx =  *pu32DirectCmdqCnt;
        pstCmdCmd->u32Addr = u32Reg;
        pstCmdCmd->u16Mask = u16Mask;
        pstCmdCmd->u16Data = u16Val;
        pstCmdCmd->u32Time = u32PollTime;
        (*pu32CmdqCmdCnt)++;
    }
}
...
void HalDispUtilityWriteDelayCmd(u16 u16PollingTime, void *pvCtxIn)
{
    ...
    if (HalDispUtilityGetRegAccessMode() ==  E_DISP_UTILITY_REG_ACCESS_CMDQ)
    {
        ...
        pstCmdCmd = (HalDispUtilityCmdqCmd_t *)(pvTblBuffer + ((*pu32CmdqCmdCnt) * sizeof(HalDispUtilityCmdqCmd_t)));
        pstCmdCmd->u32CmdqBufIdx =  *pu32DirectCmdqCnt;
        pstCmdCmd->u32Addr = HAL_DISP_UTILITY_CMDQ_DELAY_CMD;
        pstCmdCmd->u16Mask = 0xFFFF;
        pstCmdCmd->u16Data = 0xFFFF;
        pstCmdCmd->u32Time = u16PollingTime;
        (*pu32CmdqCmdCnt)++;
    }
}

bool HalDispUtilityPollWait(void *pvCmdqInf, u32 u32Reg, u16 u16Val, u16 u16Mask, u32 u32PollTime)

{
    ...
    else
    {
        ...
        {
            s32CmdqRet = pstCmdqInf->MHAL_CMDQ_CmdqPollRegBits(pstCmdqInf, u32Reg, u16Val, u16Mask, TRUE);
            if (s32CmdqRet != 0 /*MHAL_SUCCESS*/)
            {
                bRet = 0;
