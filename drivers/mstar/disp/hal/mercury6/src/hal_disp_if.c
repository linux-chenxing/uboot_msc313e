...
void __HalDispIfSetTgenRest(DrvDispCtxConfig_t *pstDispCtxCfg)
{
    ...
    if(!(pstDevContain->u32Interface & HAL_DISP_INTF_MCU) && !(pstDevContain->u32Interface & HAL_DISP_INTF_VGA))
    {
    ...
    if(pstDevContain->u32Interface & HAL_DISP_INTF_TTL)
    {
        ...
        HalDispUtilityWriteDelayCmd(10, pCmdqCtx);
...
void __HalDispIfSetRegFlipPreAct(DrvDispCtxConfig_t *pstDispCtxCfg)
{
    ...
    if(HalDispUtilityGetRegAccessMode() == E_DISP_UTILITY_REG_ACCESS_CMDQ)
    {
        ...
        if(u16CmdqInProcess == 0  && !(pstDispCtxCfg->pstCtxContain->pstDevContain[pstDispCtxCfg->u32Idx]->u32Interface & HAL_DISP_INTF_MCU) )
        {
...
void __HalDispIfSetWaitDonePostActForMcuFlm(HalDispUtilityCmdqContext_t *pstCmdqCtx)
{
    ...
    if(HalDispUtilityGetRegAccessMode() == E_DISP_UTILITY_REG_ACCESS_CMDQ)
    {
        ...
        #endif
        //HalDispUtilityW2BYTEMSKDirectCmdqWrite((void *)pstCmdqCtx);
        //HalDispUtilityPollWait((void *)pstCmdqCtx->pvCmdqInf, 0x103ea8,0x0, 0x2, 0);
...
void _HalDispIfSetRegWaitDone(void *pCtx, void *pCfg)
{
    ...
    else
    {
        ...
        HalDispUtilityPollWait((void *)pstRegWaitDoneCfg->pCmdqInf, REG_LCD_0B_L,0x200, 0x200, 0);
        #else
        //wait mode
        HalDispUtilityAddWaitCmd(pstCmdqCtx); // add wait even
        #endif
        ...
        #if CMDQ_POLL_EQ_MODE  //polling mode
        HalDispUtilityPollWait((void *)pstRegWaitDoneCfg->pCmdqInf, REG_LCD_0B_L,0x1, 0x1, 0);// add wait event  wait frame done status
        #else
        //wait mode
