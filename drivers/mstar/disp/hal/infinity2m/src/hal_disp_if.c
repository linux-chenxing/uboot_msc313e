...
HalDispQueryRet_e _HalDispIfGetInfoDevicTimeZone(void *pCtx, void *pCfg)
{
    ...
    if(u32IrqFlag == E_HAL_DISP_IRQ_TYPE_TIMEZONE_VSYNC_POSITIVE)
    {
        pstTimeZone->enType = E_HAL_DISP_TIMEZONE_BACKPORCH;
    }
    else if(u32IrqFlag == E_HAL_DISP_IRQ_TYPE_TIMEZONE_VDE_POSITIVE)
    {
        pstTimeZone->enType = E_HAL_DISP_TIMEZONE_ACTIVE;
    }
    else if(u32IrqFlag == E_HAL_DISP_IRQ_TYPE_TIMEZONE_VDE_NEGATIVE)
    {
