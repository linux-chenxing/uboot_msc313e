...
int MDrv_SPINAND_WriteCIS_for_ROM(SPINAND_FLASH_INFO_TAG_t *pSpiNandInfoTagOut)
{
    ...
    for (u16_PBA = 0; u16_PBA < 10; u16_PBA += 2)
    {
        ...
        **/
        ...
        spi_nand_msg("CIS %d is written to blk 0x%04x", u8_CISIdx, u16_PBA);
...
int MDrv_SPINAND_SearchCIS_in_DRAM(U8 *pu8_CISAddr, U8 *pu8_PNIAddr, SPINAND_FLASH_INFO_TAG_t *pSpiNandInfoTagOut)
{
    ...
    spi_nand_msg("Search CIS in DRAM");
    ...
    if(u8_IsEnd == 1)
    {
        spi_nand_err("No available CIS match with current SPINAND flash");
        return -1;
    }
    spi_nand_msg("Found CIS in given memory");
