...
//    RIU_FLAG = TRUE;
    ...
    for(u8Idx = 0; u8Idx < 10; u8Idx+=2)
    {
        ...
        {
            printf("read data to cache first != ERR_SPINAND_SUCCESS\n");
            continue;
        }
        ...
        {
            printf("Read SPINand Data != ERR_SPINAND_SUCCESS\n");
            continue;
        }
        ...
        if(u32Ret != ERR_SPINAND_SUCCESS)
        {
            printf("Read SPINand Spare Data != ERR_SPINAND_SUCCESS\n");
            continue;
        }

        if(memcmp((const void *) u8SrcAddr, (const void *) u8MagicData, sizeof(u8MagicData)) != 0)
        {
            printf("memcmp u8MagicData diff\n");
            continue;
        }
        ...
        memcpy(u8DstAddr, u8SrcAddr, sizeof(SPINAND_FLASH_INFO_t));
        {
            ...
            {
                printf("memcmp ID diff\n");
                continue;
            }

            spi_nand_msg("Found CIS in block %d", u8Idx);
        }
        return TRUE;
    }
    return FALSE;
...
MDrv_SPINAND_Init_Detect_ID:
    ...
    if(!_MDrv_SPINAND_GET_INFO())
    {
        ...
        if(u32Ret != ERR_SPINAND_SUCCESS)
        {
            spi_nand_err("Can't not Detect SPINAND Device!!!!\r");
            tSpinandInfo->au8_ID[0] = 0xFF;
            tSpinandInfo->au8_ID[1] = 0xFF;
            tSpinandInfo->u8_IDByteCnt = 2;
            return FALSE;
        }
        spi_nand_msg("Can't find CIS in block [0][2][4][6] ");
        spi_nand_msg("Device detect! MID =%x, DID =%x",u8ID[0], u8ID[1]);
...
U32 MDrv_SPINAND_Read(U32 u32_PageIdx, U8 *u8Data, U8 *pu8_SpareBuf)
{
    ...
    u32Ret = HAL_SPINAND_Read (u16ColumnAddr, PAGE_SIZE_BYTE, u8Data);
    if(u32Ret != ERR_SPINAND_SUCCESS)
    {
        return u32Ret;
    }
    ...
    // Read SPINand Spare Data
    u32Ret= HAL_SPINAND_Read(PAGE_SIZE_BYTE,SPARE_SIZE, pu8_SpareBuf);
