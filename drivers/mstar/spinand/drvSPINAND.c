...
//                memcpy(&_gtSpinandInfo, &gtSpiNandInfoTable[u32Index], sizeof(SPINAND_FLASH_INFO_t));
    ...
    if(pSpiNandDrv->tSpinandInfo.au8_ID[0] == MID_GD)
    {
        ...
        if(pSpiNandDrv->tSpinandInfo.au8_ID[1] == 0x51)
        {
...
unsigned int SPINAND_API_ReadStatusReg(unsigned char *status, unsigned char addr)
{
    unsigned char stat = 0;

    if (HAL_SPINAND_ReadStatusRegister(&stat, addr) != ERR_SPINAND_SUCCESS)
    {
        return -1;
    }
