...
static void _HalAudAtopDacSoftFunc(BOOL bEnable)
{
    U16 nMask;
    nMask = (REG_DAC_SOFT_FUNC_EN | REG_DAC_PD_R_EN | REG_DAC_PD_L_EN);

    if(bEnable)
        HalBachWriteReg(BACH_REG_BANK4, BACH_ANALOG_CTRL0e, nMask, nMask);
    else
        HalBachWriteReg(BACH_REG_BANK4, BACH_ANALOG_CTRL0e, nMask, 0);
}
...
static BOOL _HalAudAtopSwitch(AudAtopPath_e ePath, BOOL bEnable)
{
    switch(ePath)
    {
        ...
        case AUD_DAC_LINEOUT:
            if (bEnable)
            {
                //_HalAudAtopDacSoftFunc(TRUE); //must enable before DAC open, for anti pop noise
                _HalAudAtopDac(TRUE);
...
BOOL HalAudSetPadMux(AudPadMux_e PadMux, BOOL bEnable)
{
    ...
    switch(PadMux)
    {
        ...
        case AUD_PAD_MUX_DMIC:
            nConfigValue = HalBachReadReg2Byte(0x103cf2);
            nMode = _gaPadMuxMode[AUD_PAD_MUX_DMIC];
            if(bEnable)
            {
                nConfigValue |= (nMode<<DIGMIC_PADMUX_OFFSET);
                HalBachWriteReg2Byte(0x103cf2, 0xffff, nConfigValue);
            }
            else
            {
                nConfigValue &= ~(nMode<<DIGMIC_PADMUX_OFFSET);
