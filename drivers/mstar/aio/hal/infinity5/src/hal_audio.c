...
#define I2S_TDM_RX_PADMUX_OFFSET 0
#define I2S_TDM_TX_PADMUX_OFFSET 4
#define I2S_MODE_PADMUX_OFFSET 12
#define I2S_MCK_PADMUX_OFFSET 8
...
{
    AUD_DEF_DIG_PMUX,
    AUD_DEF_I2S_TDM_PMUX,
    AUD_DEF_I2S_TDM_PMUX,
    AUD_DEF_I2S_MCK_PMUX,
    AUD_DEF_I2S_MOD_PMUX
};
    ...
    switch(PadMux)
    {
        case AUD_PAD_MUX_I2S_RX:
            ...
            if(_gaPadMuxMode[AUD_PAD_MUX_I2S_MODE] == TRUE)
            {
                ...
                nMode = _gaPadMuxMode[AUD_PAD_MUX_I2S_MODE];
                if(bEnable)
                {
            ...
            {
                nConfigValue = HalBachReadReg2Byte(0x101e20);
                nMode = _gaPadMuxMode[AUD_PAD_MUX_I2S_RX];
                if(bEnable)
                {
                    _gbI2sModeRxActive = TRUE;
                    nConfigValue |= (nMode<<I2S_TDM_RX_PADMUX_OFFSET);
                    HalBachWriteReg2Byte(0x101e20, 0xffff, nConfigValue);
                }
                else
                {
                    _gbI2sModeRxActive = FALSE;
...
BOOL HalAudDmaSetRate(AudDmaChn_e eDmaChannel, AudRate_e eRate)
{
    switch(eDmaChannel)
    {
        ...
        case AUD_DMA_READER1:
            HalBachWriteReg(BACH_REG_BANK2, BACH_DMA_SRC_CFG02, BACH_DMA_RD1_SEL, 0);
            switch(eRate)
            {
                case AUD_RATE_8K:
        ...
        case AUD_DMA_WRITER2:
            ...
            {
                case AUD_RATE_8K:
                    HalBachWriteReg(BACH_REG_BANK3, BACH_VREC_CTRL04, REG_FS_SEL_MSK, 1<<REG_FS_SEL_POS);
                    break;

                case AUD_RATE_16K:
                    HalBachWriteReg(BACH_REG_BANK3, BACH_VREC_CTRL04, REG_FS_SEL_MSK, 11<<REG_FS_SEL_POS);
                    break;
        ...
        case AUD_DMA_READER2:
            HalBachWriteReg(BACH_REG_BANK2, BACH_DMA_SRC_CFG02, BACH_DMA_RD2_SEL, BACH_DMA_RD2_SEL);
            switch(eRate)
            {
                case AUD_RATE_8K:
...
BOOL HalAudDmaWrConfigMch(AudMchSet_e eMchSet, U16 nChNum, AudMchClkRef_e eMchClkRef, const int *mch_sel)
{
    ...
    switch(eMchSet)
    {
        ...
        case AUD_MCH_SET_W2:
            HalBachWriteReg(BACH_REG_BANK2, BACH_DMA_SRC_CFG01, BACH_DMA_WR_CH47_SEL_MSK, (nChSel4567<<BACH_DMA_WR_CH47_SEL_POS));
...
BOOL HalAudDigMicSetRate(AudRate_e eRate)
{
    ...
    for(i=0; i<4; i++)
    {
        HalAudDigMicSetGain(_gnDmicGain, i);
    }
...
{
    DBGMSG("%s --- %d \n",__func__,nPadMuxMode);
    _gaPadMuxMode[AUD_PAD_MUX_I2S_MODE] = nPadMuxMode;
    return TRUE;
}
