...
U32  eMMCTest_SgW_MIU(U32 u32_eMMC_Addr)
{
    ...
    for(u32_i=0; u32_i< MAX_SCATTERLIST_COUNT; u32_i++)
    {
        ...
        p_AdmaDesc_st[u32_i].u32_Address =cpu2le32(u32_dma_addr) - 0x20000000;
    ...
    REG_FCIE_W(FCIE_MIU_DMA_ADDR_31_16,((U32)((U8 *)(p_AdmaDesc_st)) >> 16) & 0X0FFF);
...
U32 eMMC_IPVerify_Main(void)
{
...
#endif
    {
        ...
        eMMC_debug(eMMC_DEBUG_LEVEL, 1, "\033[7;33meMMC IP GPIO pad: %s Mode Bus Width: %d\033[m\r\n\r\n",
                    (g_eMMCDrv.u8_PadType == FCIE_eMMC_BYPASS || g_eMMCDrv.u8_PadType == FCIE_eMMC_BYPASS_8X) ? "BYPASS" : "SDR(GPIO)",
                    u8_BusWidth[u8_i]);
