...
u8 DRV_SPINOR_write_enable(void)
{
    ...
    return DRV_SPINOR_single_transfer(&u8_cmd, 1);
}
...
static u8 _DRV_SPINOR_reset(void)
{
    ...
    if (ERR_SPINOR_SUCCESS != DRV_SPINOR_single_transfer(&u8_cmd, 1))
    {
        return ERR_SPINOR_DEVICE_FAILURE;
    }
...
u8 _DRV_SPINOR_use_ext_address(u8 u8_ext)
{
    ...
    if (ERR_SPINOR_SUCCESS != DRV_SPINOR_write_enable() || ERR_SPINOR_SUCCESS != DRV_SPINOR_single_transfer(au8_cmd, 2))
    {
        return ERR_SPINOR_DEVICE_FAILURE;
    }
...
u8 DRV_SPINOR_read_status(u8 u8_cmd, u8 *pu8_status, u8 u8_size)
{
    ...
    return DRV_SPINOR_receive_data(pu8_status, u8_size);
}
...
u8 DRV_SPINOR_write_status(u8 u8_cmd, u8 *pu8_status, u8 u8_size)
{
    if (ERR_SPINOR_SUCCESS != DRV_SPINOR_write_enable() ||
        ERR_SPINOR_SUCCESS != DRV_SPINOR_single_transfer(&u8_cmd, 1) ||
        ERR_SPINOR_SUCCESS != DRV_SPINOR_burst_transfer(pu8_status, u8_size))
    {
        return ERR_SPINOR_DEVICE_FAILURE;
    }
...
static u8 _DRV_SPINOR_program_load_data_by_bdma(u8 *pu8_data, u32 u32_size)
{
    ...
    {
        if (ERR_SPINOR_SUCCESS != DRV_SPINOR_burst_transfer(pu8_data, u32_size))
        {
            return ERR_SPINOR_DEVICE_FAILURE;
        }
...
static u8 _DRV_SPINOR_program_page(u8 u8_cmd, u32 u32_address, u8 *pu8_data, u32 u32_size)
{
    ...
    if (SPI_NOR_CMD_4PP == st_spinor_drv_info.u8_pageProgram)
    {
        DRV_FSP_use_quad_mode(1);
    }

    if (ERR_SPINOR_SUCCESS != DRV_SPINOR_burst_transfer(au8_cmd, 3))
    {
        return ERR_SPINOR_TIMEOUT;
    }

    DRV_FSP_use_quad_mode(DRV_SPINOR_is_quad_cmd(st_spinor_drv_info.u8_pageProgram));

    if (ERR_SPINOR_SUCCESS != _DRV_SPINOR_program_load_data(pu8_data, u32_size))
    {
        return ERR_SPINOR_DEVICE_FAILURE;
    }
...
static u8 _DRV_SPINOR_read_data_by_riu(u32 u32_address, u8 *pu8_data, u32 u32_size)
{
    ...
    if (ERR_SPINOR_SUCCESS != DRV_SPINOR_single_transfer(&st_spinor_drv_info.u8_readData, 1))
    {
        return ERR_SPINOR_DEVICE_FAILURE;
    }
    ...
    while (0 != u8_dummy)
    {
        ...
        {
            if (ERR_SPINOR_SUCCESS != DRV_SPINOR_burst_transfer(au8_cmd, u8_wr_size))
            {
                return ERR_SPINOR_DEVICE_FAILURE;
            }
    ...
    if (ERR_SPINOR_SUCCESS != DRV_SPINOR_receive_data(pu8_data, u32_size))
    {
        return ERR_SPINOR_DEVICE_FAILURE;
    }
...
static u8 _DRV_SPINOR_read_data_by_bdma(u32 u32_address, u8 *pu8_data, u32 u32_size)
{
    ...
    if (0 < (u32_address % 8))
    {
        u8_remainder = 8 - (u32_address % 8);

        if (u32_size < u8_remainder)
        {
            u8_remainder = u32_size;
        }

        if (ERR_SPINOR_SUCCESS != _DRV_SPINOR_read_data_by_riu(u32_address, pu8_data, u8_remainder))
        {
            return ERR_SPINOR_DEVICE_FAILURE;
        }

        u32_address += u8_remainder;
        pu8_data += u8_remainder;
        u32_size -= u8_remainder;
    }
#endif
    if (0 < u32_size)
    {
        ...
        DRV_QSPI_use_3bytes_address_mode(DRV_QSPI_cmd_to_mode(SPI_NOR_CMD_READ), 0);
    }
...
u8 DRV_SPINOR_read(u32 u32_address, u8 *pu8_data, u32 u32_size)
{
    ...
    u8 (*_pu8_def_nor_read)(u32, u8*, u32);

    _pu8_def_nor_read = _DRV_SPINOR_read;

    if (0 < ((u32)pu8_data & 0x3f) || 0 < (u32_size & 0x3f))
    {
    ...
    while (0 != u32_size)
    {
        ...
        if (ERR_SPINOR_TIMEOUT == _DRV_SPINOR_read(u32_address & SPI_NOR_16MB_MASK, pu8_data, u32_read_size))
        {
            return ERR_SPINOR_TIMEOUT;
        }
    ...
    _DRV_SPINOR_read = _pu8_def_nor_read;
...
static u8 _DRV_SPINOR_erase_status(u8 u8_command, u32 u32_address)
{
    ...
    if (ERR_SPINOR_SUCCESS != DRV_SPINOR_write_enable() || ERR_SPINOR_SUCCESS != DRV_SPINOR_single_transfer(au8_cmd, u8_wr_size))
    {
        return ERR_SPINOR_DEVICE_FAILURE;
    }
...
static u8 _DRV_SPINOR_erase(u8 u8_command, u32 u32_address)
{
    ...
    if (ERR_SPINOR_SUCCESS !=  _DRV_SPINOR_erase_status(u8_command, u32_address & SPI_NOR_16MB_MASK))
    {
        return ERR_SPINOR_DEVICE_FAILURE;
    }
    if (0 != (u32_address & ~SPI_NOR_16MB_MASK))
...
u8 DRV_SPINOR_read_id(u8 *pu8_ids, u8 u8_bytes)
{
    ...
    return DRV_SPINOR_receive_data(pu8_ids, u8_bytes);
}
...
void DRV_SPINOR_use_bdma(u8 u8_enabled)
{
    ...
    {
        _DRV_SPINOR_program_load_data = DRV_SPINOR_burst_transfer;
        _DRV_SPINOR_read = _DRV_SPINOR_read_data_by_riu;
    }
}
...
u8 DRV_SPINOR_init(void)
{
    ...
    st_spinor_drv_info.u8_readData = SPI_NOR_CMD_READ;
    st_spinor_drv_info.u8_dummy = 0x00;
    st_spinor_drv_info.u8_pageProgram = SPI_NOR_CMD_PP;
    st_spinor_drv_info.u32_time_wait = -1;

    DRV_QSPI_use_3bytes_address_mode(DRV_QSPI_cmd_to_mode(SPI_NOR_CMD_READ), 0x00);

    _DRV_SPINOR_read = _DRV_SPINOR_read_data_by_riu;
    _DRV_SPINOR_program_load_data = DRV_SPINOR_burst_transfer;

    u8_status = _DRV_SPINOR_reset_status();
    DRV_QSPI_set_timeout(0,0x0);
