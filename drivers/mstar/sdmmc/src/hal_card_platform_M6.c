/*
* hal_card_platform_M6.c- Sigmastar
*
...
void Hal_CARD_InitPADPin(PADEmType ePAD, BOOL_T bTwoCard)
{
    ...
    if(ePAD == EV_PAD1) //PAD_SD
    {
        ...
        CARD_REG_CLRBIT(GET_CARD_REG_ADDR(A_PADGPIO_BANK, 0x01), BIT07_T); //D1
        CARD_REG_CLRBIT(GET_CARD_REG_ADDR(A_PADGPIO_BANK, 0x02), BIT07_T); //D0
        CARD_REG_CLRBIT(GET_CARD_REG_ADDR(A_PADGPIO_BANK, 0x03), BIT07_T); //CLK
        CARD_REG_CLRBIT(GET_CARD_REG_ADDR(A_PADGPIO_BANK, 0x04), BIT07_T); //CMD
        CARD_REG_CLRBIT(GET_CARD_REG_ADDR(A_PADGPIO_BANK, 0x05), BIT07_T); //D3
        CARD_REG_CLRBIT(GET_CARD_REG_ADDR(A_PADGPIO_BANK, 0x06), BIT07_T); //D2
    }
...
void Hal_CARD_PullPADPin(PADEmType ePAD, PinPullEmType ePinPull, BOOL_T bTwoCard)
{
    if(ePAD == EV_PAD1) //PAD_SD
    {
        if(ePinPull ==EV_PULLDOWN)
        {
            ...
            CARD_REG_CLRBIT(GET_CARD_REG_ADDR(A_PADTOP_BANK, 0x67), BIT14_T | BIT13_T | BIT12_T); //[B13:B12]reg_sd1_mode
            CARD_REG_CLRBIT(GET_CARD_REG_ADDR(A_PADTOP_BANK, 0x67), BIT08_T | BIT09_T | BIT10_T); //[B8:B9]reg_sd0_mode [B10]reg_sd0_cdz_mode
            CARD_REG_CLRBIT(GET_CARD_REG_ADDR(A_PADTOP_BANK, 0x68), BIT14_T | BIT13_T | BIT12_T);           //[B9]reg_sd1_cdz_mode
...
void Hal_CARD_PowerOn(PADEmType ePAD, U16_T u16DelayMs)
{
    if(ePAD==EV_PAD1) //PAD_SD
    {
        CARD_REG_CLRBIT(GET_CARD_REG_ADDR(A_PADGPIO_BANK, 0x78), BIT02_T);  // output mode enable
...
BOOL_T Hal_CARD_GetGPIOState(GPIOEmType eGPIO)
{
    ...
    if( eGPIO==EV_GPIO1 ) //EV_GPIO1 for Slot 0
    {
        #if (EN_SDMMC_CDZREV)
        CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_PADGPIO_BANK, 0x00), BIT02_T);    //PAD_SD0_CDZ:reg_sd0_gpio_oen_1
        u16Reg = CARD_REG(GET_CARD_REG_ADDR(A_PADGPIO_BANK, 0x00)) & BIT00_T; //PAD_SD0_CDZ:reg_sd0_gpio_in_1
        #else
        u16Reg = CARD_REG(GET_CARD_REG_ADDR(A_PADGPIO_BANK, 0x00)) & BIT00_T;
        #endif
