...
void HAL_MSPI_Init(struct mstar_spi *bs,MSPI_CH eChannel,u8 u8Mode)
{
    ...
    {
        mspi_errmsg("mspi0 pad mode must be 1~4\n");
        return;
    }
    else if((eChannel == E_MSPI1) && (u8Mode > 5))
    {
        mspi_errmsg("mspi1 pad mode must be 1~5\n");
        return;
    }
...
{
    struct mstar_spi *bs = to_ss_spi_slave(slave);
    if (gpio_padode < PINMUX_FOR_SPI0_MODE_1 || gpio_padode > PINMUX_FOR_SPI1_MODE_5)
        return;
