...
//    ""
//);
#if !defined(CONFIG_MS_NAND_ONEBIN) && !defined(CONFIG_MS_NOR_ONEBIN)
...
#if defined(CONFIG_SSTAR_AIO)
extern void lineout_start(void);
extern void lineout_gain(int nGain);

int do_lineout(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    int gain = 0;
...
U_BOOT_CMD(
     lineout,  CONFIG_SYS_MAXARGS,    1,    do_lineout,
     "audio line-out start\n", ""
 );
#endif
